package com.chongdao.client.config;

public class AliPayConfig {

    public static final String ALI_PAY_APPID = "2019072365944369"; // appid

    // app支付应用私钥
    public static final String APP_PRIVATE_KEY = "";

    // app应用公钥
    public static final String ALI_PAY_SPLICATION_PUBLIC_KEY = ""; // 支付宝公钥

    //支付宝公钥
    public static final String ALI_PAY_PUBLIC_KEY = "";

    //回调
    public static final String NOTIFY_URL = "http://47.100.63.167/app_client/api/pay/aliPayCallback";

    public static final String NOTIFY_URL_HT = "http://47.100.63.167/app_client/api/pay/aliPayCallbackHt";
    //保险医疗费用支付宝支付回调
    public static final String NOTIFY_URL_INSURANCE = "https://dev.chongdaopet.com/app_client/api/pay/aliPayCallbackInsurance";
     // 支付完成跳转页面
    public static final String RETURN_URL = "";
    // 请求网关地址（）
    //沙箱网关https://openapi.alipaydev.com/gateway.do
    public static final String GATEWAY = "https://openapi.alipay.com/gateway.do";
    // 编码格式
    public static final String CHARSET = "UTF-8";
    // 返回格式
    public static final String FORMAT = "json";
    // RSA2
    public static final String SIGN_TYPE = "RSA2";







}
