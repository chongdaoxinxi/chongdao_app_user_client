...
//	this(input, "UTF8");
    ...
    public static String encodeByMd5(String s,String charset) throws Exception {
        if(charset == null || charset.equals("")){
            charset = "UTF-8";
        }
        MessageDigest md = MessageDigest.getInstance("Md5");
        md.reset();
        byte cryptbyte[] = md.digest(s.getBytes(charset));
        return byteToString(cryptbyte);
    }
     ...
     */
    public final static String encryptByMD5(String ori, String key) throws Exception {
        return MD5_SHA("MD5", ori + key);
    }

    /**
     * MD5、SHA加密
     *
     * @param encryptionType 要加密的类型 (MD5、SHA1、SHA-256、SHA-384、SHA-512)
     * @param s              要加密的数据
     * @return 加密后的数据
     * @throws GeneralSecurityException
     */
    public final static String MD5_SHA(String encryptionType, String s) throws Exception {
     ...
     * @throws Exception
     */
    public static String cqMD5(String plainCode) {
        String strDig = "";
        try {
            MessageDigest md = MessageDigest.getInstance("MD5");
            md.update(plainCode.getBytes("UTF8"));
            byte[] byteStr = md.digest();

            for (int i = 0; i < byteStr.length; i++) {
                String hexStr = Integer.toHexString(0x000000ff & byteStr[i] | 0xffffff00).substring(6);
                strDig += hexStr;
            }
        } catch (Exception e) {
            e.getMessage();
    ...
    public static void main(String[] args) throws Exception {
        String orderId = "350216101513395813";
        String source = "510";
        String title = "全家宝家庭组合保险产品";
        String tradeAmount = "690.0";
        String tradeType = "WX_JSAPI";
        String key = "1QA2WEPICCD4RF5tg6";
        String all = "";
        all = orderId + source + title + tradeAmount + tradeType + key;
        System.out.println("all====" + all);
        String miyue = Md5.MD5_SHA("MD5", all);
        System.out.println("m iyue=====" + miyue);
    }
}
