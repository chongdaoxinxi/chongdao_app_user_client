        ...
        if (token != null) {
            ...
            try {
                ...
                if(username == null || username.isEmpty()){
                    resp.put("ERR_MSG", ResultEnum.USERNAME_NOT_EMPTY);
                    return resp;
                }
                //账号在别处登录
                if(userRepository.findByName(username).getLastLoginTime().after(generateTime)){
                    resp.put("ERR_MSG", ResultEnum.USER_LOGIN_ALREADY);
                    return resp;
                }
                ...
                // don't trust the JWT!
                // jwt 解析错误
                resp.put("ERR_MSG",ResultEnum.TOKEN_ERROR);
                return resp;
            } catch (ExpiredJwtException e) {
                // TODO: handle exception
                // jwt 已经过期，在设置jwt的时候如果设置了过期时间，这里会自动判断jwt是否已经过期，如果过期则会抛出这个异常，我们可以抓住这个异常并作相关处理。
                resp.put("ERR_MSG",ResultEnum.TOKEN_EXPIRED);
                return resp;
            }
        }else {
            resp.put("ERR_MSG",ResultEnum.TOKEN_NOT_EMPTY);
            return resp;
        }
    }
